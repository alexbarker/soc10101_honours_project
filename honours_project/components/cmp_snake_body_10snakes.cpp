...
#include "../define.h"
#include "cmp_snake_body_10snakes.h"
...
void Snake10::setPath(const std::vector<sf::Vector2i>& p) {
	path = p;
}

void Snake10::clearPath() {
	path.clear();
}
...
void Snake10::display(RenderWindow& win, Config& config) {
...
bool Snake10::isOutOfBounds() const {
...
bool Snake10::hasPath() const {
	return !path.empty();
}

int Snake10::getScore() const {
	return score;
}

bool Snake10::isDead() const {
	return dead;
}

Color Snake10::getColor() const {
	return color;
}

Vector2i Snake10::getHeadPosition() const  {
	return pos[0];
}

std::vector<Vector2i> Snake10::getPosition() const {
	return pos;
}

int Snake10::getPathSize() const {
