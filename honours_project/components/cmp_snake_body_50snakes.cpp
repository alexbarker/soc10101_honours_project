...
#include "../define.h"
#include "cmp_snake_body_50snakes.h"
...
void Snake50::setPath(const std::vector<sf::Vector2i>& p) {
	path = p;
}

void Snake50::clearPath() {
	path.clear();
}
...
void Snake50::display(RenderWindow& win, Config& config) {
...
bool Snake50::isOutOfBounds() const {
...
bool Snake50::hasPath() const {
	return !path.empty();
}

int Snake50::getScore() const {
	return score;
}

bool Snake50::isDead() const {
	return dead;
}

Color Snake50::getColor() const {
	return color;
}

Vector2i Snake50::getHeadPosition() const  {
	return pos[0];
}

std::vector<Vector2i> Snake50::getPosition() const {
	return pos;
}

int Snake50::getPathSize() const {
