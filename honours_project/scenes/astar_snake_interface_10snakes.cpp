...
#include <iostream>
#include "astar_snake_interface_10snakes.h"
...
void Gui10::sortByScore(Snake10 snake[NB_PLAYER10]) {
	for (int i = 0; i < NB_PLAYER10; i++) {
		scoreMap.emplace(snake[i].getScore(), i);
	}
}

void Gui10::display(RenderWindow& win, std::vector<sf::Vector2i> fixedO, Snake10 snake[NB_PLAYER10], Crystal10 crystal[NB_CRYSTAL10], Config& config) {
	sortByScore(snake);
		...
		for (int i = 0; i < G_WIDTH; i++) {
			for (int j = 0; j < G_HEIGHT; j++) {
				...
				if(config.isDisplayWeightOn())
					color += 128 * (1 - Astar10::getWeight(i, j) / WEIGHT_REF);
				if(config.isDisplayCostOn())
					color += 128 * (1 - Astar10::getCost(i, j) / WEIGHT_REF);
	...
	if (NB_PLAYER10 > 10) {
		if (scrollVal < 6 * W_HEIGHT / 7 - (int)(W_HEIGHT / 12 * (NB_PLAYER10 + 1)))
			scrollVal = 6 * W_HEIGHT / 7 - (int)(W_HEIGHT / 12 * (NB_PLAYER10 + 1));
	}
	else
		scrollVal = 0;
