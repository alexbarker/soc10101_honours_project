#ifndef __ASTAR50_H__
#define __ASTAR50_H__
...
class Astar50 {
	private:
		class Node50 {
			private:
				Node50* parent;						// To know where we come from and find path back once we reached the destination
				std::vector<Node50*> neighbors;		// Pointers to the grid neighbors
			...
			public:
				Node50();
				Node50(sf::Vector2i& v, const std::vector<Node50*>& nb, Node50* p = NULL, float ec = 0, float w = 0, bool vis = false);
				void setParent(Node50* p);
				void setCost(float ec, float csf);
				void setWeight(float w);
				void setVisited(bool v);
				Node50* getParent() const;
				std::vector<Node50*> getNeighbors() const;
				...
				class Comparator {
					public:
						bool operator()(const Node50* a, const Node50* b);
				};
		};

		static sf::Vector2i from;					// Departure
		static sf::Vector2i to;						// Destination
		static Node50 graph[G_WIDTH][G_HEIGHT];		// Map grid
