...
#include "../define.h"
#include "astar_snake_launcher_1snake.h"
...
Game1::Game1() {
	...
	Astar1::setTo(crystal[0].getPosition(), config);
}

void Game1::play(RenderWindow& win) {
	...
	while (win.isOpen()) {
		...
		if (config.isRunning()) {
			...
			if (config.isWeightBFSOn())
				Astar1::hardReset(obstacles);
			else
				Astar1::softReset(obstacles);

			generateObstacles();
			for (int i = 0; i < NB_PLAYER1; i++) {
				if (!snake[i].isDead()) {
					// Set departure point
					Astar1::setFrom(snake[i].getHeadPosition());

					if (NB_CRYSTAL1 == 1) {
						snake[i].setPath(Astar1::findPath(config));
					}
					else {
						snake[i].clearPath();
						std::vector<Vector2i> path;
						//Calculate path to every crystal and keep the shortest
						for (int j = 0; j < NB_CRYSTAL1; j++) {
							Astar1::setTo(crystal[j].getPosition(), config);
							path = Astar1::findPath(config);
					...
					if (collide(snake[i])) {
						...
						Astar1::update(snake[i].getHeadPosition());
					}
				}
			}
		}
...
void Game1::gameOver(sf::RenderWindow& win) {
	handleEvents(win);
...
void Game1::handleEvents(sf::RenderWindow& win) {
	Event event;
	while (win.pollEvent(event)) {
